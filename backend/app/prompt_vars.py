...
def format_audit_judge_brief(
    *,
    ...
    parts: list[str] = [
        ...
        "   - 若涉及数值限值/比较符/允许偏差，必须先完成下方「数值限值强制核对」，"
        "再给出 status；不得跳过核对直接凭语感判定 supported。",
        ...
        "",
        "数值限值强制核对（报告与标准均可抽出数值/比较符时必须逐项完成）：",
        "A. 比较符/方向：分别写出报告与标准的比较符（≤、<、≥、>、±、区间等）。",
        "   - 上限类（≤/<）与下限类（≥/>）方向相反 → 一律 mismatch"
        "（例如标准 ≤55、报告 ≥55；标准 ≥40、报告 ≤40）。",
        "   - 不得把 ≥ 当成 ≤、把“不低于”当成“不高于”，也不得忽略符号只比数字。",
        "B. 单位归一：先把 % / 百分数 / 小数（如 1% 与 0.01）换算到同一量纲再比；"
        "单位等价且数值等价、方向相同 → 可 supported。",
        "C. 松紧比较（仅在同一比较方向、同一量纲下）：",
        "   - 上限类（≤/<）：报告数值 > 标准数值 → 报告更宽 → mismatch；"
        "报告数值 ≤ 标准数值 → 不宽于标准 → supported。"
        "例：标准 ≤0.370、报告 ≤0.500 → mismatch（不是 supported）。",
        "   - 下限类（≥/>）：报告数值 < 标准数值 → 报告更宽 → mismatch；"
        "报告数值 ≥ 标准数值 → 不宽于标准 → supported。"
        "例：标准 ≥40、报告 ≥35 → mismatch。",
        "   - ± 偏差：报告允许带宽大于标准 → mismatch；小于或等于 → supported。"
        ...
        "- 数值限值类须在 reason 中写出比较符与松紧结论，不得含糊带过。",
