...
def build_report(
    ...
    *,
    ...
    return {
        ...
        "gold_status": "candidate_pending_domain_review",
        "metric_semantics": {
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Retrieval v2 Multi-Metric Evaluation",
        "",
        f"Status: `{report['status']}`",
        f"Cases: `{report['progress']['completed_cases']}/{report['progress']['total_cases']}`; errors: `{report['progress']['error_cases']}`",
        "",
        "> Candidate benchmark only: the evidence relations are corpus-verified but pending domain approval.",
        "",
        "## Overall",
        "",
...
def main() -> None:
    ...
    for index, case in enumerate(eligible, start=1):
        ...
        try:
            completed.append(evaluate_case(case, queries_by_id[case_id]["production"]))
