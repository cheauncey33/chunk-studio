...
DEFAULT_DATASET = ROOT / "evaluation" / "retrieval_ground_truth_v2_draft.json"
DEFAULT_CASE_POOL = ROOT / "evaluation" / "retrieval_case_pool_v1.json"
DEFAULT_DB = ROOT / "backend" / "data" / "chunkstudio.db"
SCHEMA = ROOT / "backend" / "schemas" / "retrieval_ground_truth_v2.schema.json"
...
def _semantic_errors(dataset: dict[str, Any], case_pool: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    cases = dataset.get("cases", [])
    expected_ids = {case["case_id"] for case in case_pool.get("cases", [])}
    actual_ids = [case.get("case_id") for case in cases]
    if len(actual_ids) != len(set(actual_ids)):
        errors.append("case IDs must be unique")
    if set(actual_ids) != expected_ids:
        errors.append("dataset case IDs must exactly match the source case pool")
...
def validate_dataset(
    ...
    case_pool_path: Path = DEFAULT_CASE_POOL,
    db_path: Path | None = DEFAULT_DB,
    *,
    verify_corpus: bool = True,
) -> dict[str, int | bool]:
    dataset = _read_json(dataset_path)
    case_pool = _read_json(case_pool_path)
    ...
    errors.extend(_semantic_errors(dataset, case_pool))
